    ...
    if (!stats.empty())
    {
        ...
        for(auto itr = stats.begin(); itr != stats.end(); ++itr){
            ...
            case /* constant-expression */0:
                /* code */
                throughput1 << curTime.GetSeconds() << " "
                << 8 * (itr->second.rxBytes - prev[i]) / ((curTime - prevTime).ToDouble(Time::US))
                << std::endl;
                prev[i] = itr->second.rxBytes;
                break;
            case 2:
                throughput2 << curTime.GetSeconds() << " "
                << 8 * (itr->second.rxBytes - prev[i]) / ((curTime - prevTime).ToDouble(Time::US))
                << std::endl;
                prev[i] = itr->second.rxBytes;
                break;
            case 4:
                throughput3 << curTime.GetSeconds() << " "
                << 8 * (itr->second.rxBytes - prev[i]) / ((curTime - prevTime).ToDouble(Time::US))
                << std::endl;
                prev[i] = itr->second.rxBytes;
                break;
...
{
  std::string transport_prot = "QuicRlTimeBased";
  double error_p = 0.0;
  ...
  std::string queue_disc_type = "ns3::PfifoFastQueueDisc";
    ...
    if (transport_prot == "QuicRlTimeBased" or transport_prot == "TcpRlTimeBased")
    {
  ...
  for (uint16_t i = 0; i < sources.GetN (); i++)
    {
      ...
      clientApps[i] = ftp.Install (sources.Get (i));
      clientApps[i].Stop (Seconds (59.0+i*20));
      clientApps[i].Start (Seconds (0.1+i*20));
  ...
  throughput1.open("throughput1.dat", std::ios::out);
  throughput2.open("throughput2.dat", std::ios::out);
  throughput3.open("throughput3.dat", std::ios::out);
  ...
  for (std::map<FlowId, FlowMonitor::FlowStats>::const_iterator i = stats.begin (); i != stats.end (); ++i)
  {
    ...
    std::cout << "  TxOffered:  " << i->second.txBytes * 8.0 / stop_time / 1000 / 1000  << " Mbps\n";
    std::cout << "  Rx Packets: " << i->second.rxPackets << "\n";
    std::cout << "  Rx Bytes:   " << i->second.rxBytes << "\n";
    std::cout << "  Throughput: " << i->second.rxBytes * 8.0 / (stop_time-2) / 1000000 << " Mbps\n";
  }
