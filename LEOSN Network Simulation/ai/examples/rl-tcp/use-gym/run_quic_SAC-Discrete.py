...
try:
    ...
    while True:
        ...
        if args.show_log:
            print("Recv obs:",
                  [ssThresh, cWnd / segmentSize, throughput / 100000, max(throughput_record) / 100000, max_throughput,
                   rtt, min_rtt, bytesInFlight, cwnd_bdp_estimate, max(throughput_record) * min_rtt / 10000000,
                   loss_rate])
        ...
        if rtt > 0:
            # reward = ((1 - loss_rate) * (throughput**2) / rtt) / (max(throughput_record)**2 / min_rtt)
