...
#ifndef NS3_QUIC_RL_ENV_H
#define NS3_QUIC_RL_ENV_H
...
class QuicEnvBase : public OpenGymEnv
{
  public:
    QuicEnvBase();
    ~QuicEnvBase() override;
    ...
    // 可能有问题
    virtual void TxPktTrace(Ptr<const Packet>, const QuicHeader&, Ptr<const QuicSocketBase>) = 0;
    virtual void RxPktTrace(Ptr<const Packet>, const QuicHeader&, Ptr<const QuicSocketBase>) = 0;
...
class QuicTimeStepEnv : public QuicEnvBase
{
  public:
    QuicTimeStepEnv();
    ~QuicTimeStepEnv() override;
    static TypeId GetTypeId();
    void DoDispose() override;
    ...
    // trace packets, e.g. for calculating inter tx/rx time
    void TxPktTrace(Ptr<const Packet>, const QuicHeader&, Ptr<const QuicSocketBase>);
    void RxPktTrace(Ptr<const Packet>, const QuicHeader&, Ptr<const QuicSocketBase>);
...
class QuicEventBasedEnv : public QuicEnvBase
{
  public:
    QuicEventBasedEnv();
    ~QuicEventBasedEnv() override;
    static TypeId GetTypeId();
    void DoDispose() override;
