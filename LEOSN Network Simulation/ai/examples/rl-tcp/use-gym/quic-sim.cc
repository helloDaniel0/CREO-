...
std::string quicTypeId = "QuicRlTimeBased";
// std::string quicTypeId = "QuicBbr";
// std::string quicTypeId = "QuicCubic";
// std::string quicTypeId = "QuicVegas";
std::string dir = "results/" + quicTypeId + "-download" + "/";;
...
void PhysicalLayerHandover(NetDeviceContainer sat_gs, bool start){
    ...
    Ptr<RateErrorModel> em = CreateObjectWithAttributes<RateErrorModel>("ErrorRate", DoubleValue(error_rate), 
                            "ErrorUnit", EnumValue(RateErrorModel::ERROR_UNIT_PACKET));
    relay->SetAttribute("ReceiveErrorModel", PointerValue(em));
    relay->GetObject<PointToPointNetDevice> ()->SetAttribute ("DataRate", StringValue("8Mbps"));
    wbw << Simulator::Now().GetSeconds() << " "<< 0 << std::endl;
    }
}

void StopAndWait(Ptr<QuicSocketBase> &tcpsb, bool start, NetDeviceContainer sat_gs){
    Ptr<NetDevice> relay = sat_gs.Get(0);
    if (start == true)
    {
        tcpsb->SetSeamlessHO(start, relay);
        tcpsb->SetStopandResumePacing(DataRate("1kbps"));
    }else{
        tcpsb->SetSeamlessHO(start, relay);
        tcpsb->SetStopandResumePacing(DataRate("4Gbps"));
    }
}
...
void ChangeP2PAttributesPeriodically (NetDeviceContainer dish_sat, NetDeviceContainer sat_gs, Ptr<QuicL4Protocol> &tcp, Ptr<QuicSocketBase> &tcpsb, NodeContainer tcpsender, bool start) {
    ...
    tcpsb->SetCapacityLatency(std::min(bw1,bw2), (delay1+delay2)/100);


    if (switch_flag == 1 && start == false)
    {
        if(isSeamless){
            Simulator::Schedule (Seconds (0.05+0.00001), &StopAndWait, tcpsb, true, sat_gs);
            Simulator::Schedule (Seconds (0.05+0.00001+30*tcpsb->latency), &StopAndWait, tcpsb, false, sat_gs);
        }

        Simulator::Schedule (Seconds (0.05), &PhysicalLayerHandover, sat_gs, true);
        Simulator::Schedule (Seconds (0.05+3*tcpsb->latency), &PhysicalLayerHandover, sat_gs, false);
        Simulator::Schedule (Seconds (next - now), &ChangeP2PAttributesPeriodically, sender1, sender2, tcp, tcpsb, tcpsender, true);
    }else
    {
        ...
        Simulator::Schedule (Seconds (next - now), &ChangeP2PAttributesPeriodically, sender1, sender2, tcp, tcpsb, tcpsender, false);
    }
...
main(int argc, char* argv[])
{
    ...
    Config::SetDefault("ns3::QuicL4Protocol::SocketType", StringValue("ns3::" + quicTypeId));

    Ptr<OpenGymInterface> openGymInterface;
    if (quicTypeId == "QuicRlTimeBased" or quicTypeId == "TcpRlTimeBased")
    {
    ...
    dishes_leosat_link.SetDeviceAttribute("DataRate", StringValue("10Mbps"));
    dishes_leosat_link.SetChannelAttribute("Delay", StringValue("2ms"));

    PointToPointHelper leosat_gs_link;
    leosat_gs_link.SetDeviceAttribute("DataRate", StringValue("20Mbps"));
